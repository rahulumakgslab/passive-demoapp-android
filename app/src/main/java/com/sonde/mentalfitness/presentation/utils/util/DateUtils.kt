...
fun getTimeInReadableFormat(seconds: Int): String {
    var timeInReadableFormat = ""
    ...
    if (hour > 1) {
        if (minutes > 1) {
            ...
            timeInReadableFormat="${hour}hr ${minutes}min ${secondsToDisplay}sec"
        }else{
            timeInReadableFormat="$seconds sec"
        }
    } else {
        if (minutes > 0) {
            val secondsToDisplay=seconds % 60
            timeInReadableFormat="${minutes}min ${secondsToDisplay}sec"
        }else{
            timeInReadableFormat="${seconds}sec"
        }
    }
    return timeInReadableFormat
}
