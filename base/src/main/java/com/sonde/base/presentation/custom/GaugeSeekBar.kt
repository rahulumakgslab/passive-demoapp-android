...
import com.sonde.base.presentation.utils.util.getContextColor

/**
 *  https://github.com/10clouds/GaugeSeekBar
 */
...
class GaugeSeekBar : View {
    ...
    private var thumbColor: Int = ContextCompat.getColor(context, R.color.track_color1)
    ...
    fun setCustomThumbDrawable(customThumbDrawable: ThumbDrawable) {
        this.customThumbDrawable = customThumbDrawable
        this.thumbEntity?.setThumbDrawable(customThumbDrawable)
        invalidate()
    }
    ...
    private fun applyAttributes(attributes: TypedArray) {
        try {
            ...
            if (trackGradientArrayPositionsResourceId != 0) {
                val positionsIntArray = resources.getStringArray(trackGradientArrayPositionsResourceId)
                progressGradientArrayPositions =
                    FloatArray(positionsIntArray.size) {
                        positionsIntArray[it].toFloat()
//                        positionsIntArray[it].div(100f)
                    }
            }
        ...
        if (showThumb) {
            val thumbDrawable =
                if (thumbDrawableId != 0) {
                    ContextCompat.getDrawable(context, thumbDrawableId)!!
                } else if (customThumbDrawable != null) {
//                    ThumbDrawable(getColorByRange(progress, minRange, maxRange))
                    customThumbDrawable!!
                } else {
                    ThumbDrawable(getContextColor(R.color.colorAccent))
                }
...
//        val range = (maxRange - minRange) / 3
//        if (progress < (minRange + range)) {
//            val startColor = ContextCompat.getColor(context, R.color.track_color1)
//            val endColor = ContextCompat.getColor(context, R.color.track_color2)
//            return ColorUtils.blendARGB(startColor, endColor, 0.5f)
//
//        } else if (progress < (minRange + 2 * range)) {
//            val startColor = ContextCompat.getColor(context, R.color.track_color2)
//            val endColor = ContextCompat.getColor(context, R.color.track_color3)
//            return ColorUtils.blendARGB(startColor, endColor, 0.5f)
//
