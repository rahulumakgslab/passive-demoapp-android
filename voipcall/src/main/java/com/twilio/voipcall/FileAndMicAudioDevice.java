...
public class FileAndMicAudioDevice implements AudioDevice {
    ...
    private final Runnable microphoneCapturerRunnable = () -> {
        ...
        while (true) {
            ...
            if (bytesRead == micWriteBuffer.capacity()) {
                ...
                if (isCallConnected() && !fileManager.isCompleteFileCaptured()) {
                    fileManager.writePCMData(micWriteBuffer, false);
                }
            } else {
    ...
    public synchronized boolean isCallConnected() {
        return isCallConnected;
    }

    public synchronized void setCallConnected(boolean callConnected) {
        isCallConnected = callConnected;
    }

    public synchronized void stopCallRecording() {
